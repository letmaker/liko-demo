...
async function test() {
  ...
  apple.pos.set(500, 100); // 设置苹果的初始位置，坐标为(500,100)
  app.stage.addChild(apple); // 将苹果添加到游戏场景中

  // 创建一个复杂的动画序列：苹果的移动和透明度变化
  const tween = Tween.from({ target: apple, props: { alpha: 0, pos: { x: 100, y: 100 } } }) // 设置动画起始状态：完全透明，位置在(100,100)
    .to({
  ...
  strawberry.pos.set(100, 500); // 设置草莓的初始位置
